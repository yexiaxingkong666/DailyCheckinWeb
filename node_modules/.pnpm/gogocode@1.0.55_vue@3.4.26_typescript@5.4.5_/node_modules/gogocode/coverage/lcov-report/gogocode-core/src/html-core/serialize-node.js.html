...
<head>
    <title>Code coverage report for gogocode-core/src/html-core/serialize-node.js</title>
    <meta charset="utf-8" />
...
<div class='wrapper'>
    <div class='pad1'>
        <h1><a href="../../../index.html">All files</a> / <a href="index.html">gogocode-core/src/html-core</a> serialize-node.js</h1>
        <div class='clearfix'>
...
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-neutral">&nbsp;</span>
<span class="cline-any cline-yes">37x</span></td><td class="text"><pre class="prettyprint lang-js">const {
    NODE_DOCUMENT,
    NODE_DOCTYPE,
    NODE_TAG,
    NODE_TEXT,
...
function serializeTagAttributes (attributes = []) {
    ...
    return attributes.map((item) =&gt; {
        ...
        <span class="missing-if-branch" title="else path not taken" >E</span>if (item.key !== undefined) {
        ...
        // 处理属性中的引号
            ...
            <span class="missing-if-branch" title="if path not taken" >I</span>if (item.value.content &amp;&amp; item.value.content.match &amp;&amp; item.value.content.match(quota)) {
<span class="cstat-no" title="statement not covered" >                quota = ["'", "'"];</span>
            }
&nbsp;
            <span class="missing-if-branch" title="if path not taken" >I</span>if (item.value.content &amp;&amp; item.value.content.trim &amp;&amp; item.value.content.trim()[0] == '=') {
                // 处理属性中的if语句{{#if(xx == bb)}}xxx{{/if(xx == bb)}}
<span class="cstat-no" title="statement not covered" >                quota = ['', '']</span>
                // serialized += `=${ item.value.content }`
            } else <span class="missing-if-branch" title="if path not taken" >I</span>if (item.value.content &amp;&amp; item.value.content.match &amp;&amp; item.value.content.match(/\(/) &amp;&amp; item.value.content.match(/\)/) &amp;&amp; item.key.content.match(/\{\{/)) {
...
function serializeDoctypeAttributes (attributes = <span class="branch-0 cbranch-no" title="branch not covered" >[])</span> {
    return attributes.map((item) =&gt; {
        let wrapper = ''
  
        <span class="missing-if-branch" title="if path not taken" >I</span>if (item.startWrapper !== undefined) {
<span class="cstat-no" title="statement not covered" >            wrapper = item.startWrapper</span>
        }
...
function serializeNode (node, serializedChildren = '') {
    ...
    case NODE_SCRIPT: {
        return serializeTagNode(
            'script',
            node.content.attributes,
            node.content.value ? node.content.value.content : <span class="branch-1 cbranch-no" title="branch not covered" >'',</span>
            false,
            node
        )
    }
...
module.exports = serializeNode</pre></td></tr></table></pre>
